...
Dữ liệu đầu vào được thu nhận trực tiếp từ webcam (camera máy tính) trong thời gian thực, với độ phân giải 640×480 pixel và tần số 30 FPS. Hệ thống xử lý từng frame BGR thu được từ webcam và đưa ra dự đoán trạng thái của tài xế ngay lập tức. Dữ liệu kiểm tra cho notebook thí nghiệm sử dụng ảnh tĩnh chụp từ webcam trong điều kiện ánh sáng phòng học thông thường (300–500 lux). Toàn bộ thí nghiệm được thực hiện trên máy tính có CPU Intel Core i5, RAM 8GB, không có GPU.
...
Abayomi và cộng sự (2022) [4] phát triển hệ thống phát hiện buồn ngủ lái xe kết hợp EAR, MAR và head pose, đạt độ chính xác 94.7% trên tập dữ liệu thu thập từ 20 tài xế. So với nghiên cứu này, đồ án của chúng tôi bổ sung thêm Canny edge detection và phân tích iris bằng connected components, cùng với cơ chế dynamic threshold để thích nghi với từng người dùng.
...
**Head Pose (Góc tư thế đầu):** Được ước lượng từ tọa độ landmark mà không cần camera depth. Roll angle (góc xoay ngang đầu) được tính bằng góc giữa vector nối tâm hai mắt (trái: điểm 36, phải: điểm 45) và phương ngang (trục X): roll = atan2(eye_center_y_diff, eye_center_x_diff). Pitch angle (góc cúi hoặc ngửa đầu) được ước lượng dựa trên tỷ lệ giữa chiều cao sống mũi (từ điểm 27 đến điểm 30) và chiều cao khuôn mặt (từ lông mày đến cằm). Khi cúi đầu, tỷ lệ này giảm; khi ngửa đầu, tỷ lệ này tăng.
...
Ngáp (Yawn) được phát hiện khi MAR > YAWN_THRESHOLD (0.30) trong 5 frame liên tiếp, có cooldown 4 giây giữa các lần phát hiện. Nghiêng đầu (Head Tilt) được phát hiện khi Roll angle > HEAD_TILT_THRESHOLD (15°) hoặc Pitch > HEAD_TILT_THRESHOLD (15°) so với giá trị tham chiếu ban đầu, trong 20 frame liên tiếp. Mệt mỏi (Fatigue) được cảnh báo khi tần suất chớp mắt > 25 lần/phút hoặc tần suất ngáp > 3 lần/phút. Mất tập trung (No Face) được phát hiện khi không thấy khuôn mặt trong 20 frame liên tiếp.
...
Thí nghiệm được thực hiện trên các ảnh tĩnh chụp từ webcam (640×480) và trực tiếp trên luồng webcam thời gian thực với các trạng thái: tỉnh táo, nhắm mắt, ngáp, nghiêng đầu. Hệ thống sử dụng metrics collector để ghi lại EAR, MAR, roll angle, pitch angle cho mỗi frame nhằm quan sát hành vi của các chỉ số ở từng ngưỡng khác nhau.
...
|-----------|:---------:|:------:|:--------:|----------|
| 0.16 | Cao | Thấp | Thấp | Ngưỡng quá thấp, bỏ sót nhiều frame nhắm mắt |
| 0.18 | Cao | Trung bình | Trung bình | Cải thiện độ nhạy nhưng vẫn còn bỏ sót |
| 0.20 | Trung bình | Trung bình | Trung bình | Cân bằng hơn nhưng chưa tối ưu |
| 0.22 | Cao | Cao | **Tốt nhất** | Cân bằng precision-recall tốt nhất |
| 0.24 | Trung bình | Cao | Khá | Xuất hiện dương tính giả |
| 0.26 | Thấp | Rất cao | Thấp | Nhiều dương tính giả do mắt mở bị phân loại sai |
...
|-----------|:---------:|:------:|:--------:|----------|
| 0.25 | Thấp | Cao | Trung bình | Nhiều dương tính giả — nhầm nói chuyện/cười thành ngáp |
| 0.30 | Cao | Cao | **Tốt nhất** | Cân bằng precision-recall tốt nhất |
| 0.35 | Cao | Trung bình | Tốt | Bỏ sót một số cơn ngáp |
| 0.40 | Cao | Thấp | Trung bình | Bỏ sót nhiều cơn ngáp, không phù hợp thực tế |
...
|:----------------:|:---------:|:------:|:--------:|----------|
| 10° | Thấp | Cao | Trung bình | Quá nhạy — nhiều dương tính giả |
| 15° | Cao | Cao | **Tốt nhất** | Cân bằng tốt, ưu tiên an toàn |
| 20° | Cao | Trung bình | Khá | Bỏ sót nhiều trường hợp nghiêng đầu |
...
|-----------|-----------------|----------|
| EAR càng thấp → độ nhạy giảm, precision tăng | EAR = 0.16 bỏ sót nhiều, EAR = 0.26 nhiều dương tính giả | Đúng |
| MAR = 0.30 cho cân bằng precision-recall tốt nhất | 0.25 nhiều dương tính giả; 0.40 bỏ sót; 0.30 cân bằng nhất | Đúng |
| HEAD_TILT = 15° cho kết quả phù hợp nhất | 10° quá nhạy; 20° bỏ sót; 15° cân bằng, ưu tiên an toàn | Đúng một phần |
| Pipeline tổng thể đáp ứng yêu cầu thời gian thực | Phát hiện chính xác ba trạng thái, độ trễ thấp | Đúng |

Giả thuyết thứ ba (HEAD_TILT = 15°) được xác nhận là "đúng một phần": ngưỡng 20° cho precision cao hơn nhưng bỏ sót nhiều trường hợp nghiêng đầu. Nhóm chọn ngưỡng 15° vì ưu tiên an toàn.
...
**NMS và Otsu threshold:** Việc kết hợp hai phương pháp phát hiện khuôn mặt (Haar Cascade, DNN SSD) với NMS giúp tăng độ tin cậy. Otsu tự động chọn ngưỡng dựa trên histogram của từng frame, giúp phát hiện cạnh nhất quán hơn khi ánh sáng thay đổi.
...
Kết quả quan sát khẳng định 3/4 giả thuyết là **đúng** và 1/4 là **đúng một phần**.
...
**Giả thuyết 3 — HEAD_TILT = 15° cho kết quả phù hợp nhất:** Đúng một phần. Ngưỡng 20° cho precision cao hơn nhưng bỏ sót nhiều trường hợp nghiêng đầu. Nhóm chọn 15° vì ưu tiên an toàn.

**Giả thuyết 4 — Pipeline tổng thể đáp ứng yêu cầu:** Đúng. Hệ thống phát hiện chính xác ba trạng thái với độ trễ thấp, đáp ứng yêu cầu thời gian thực.
...
| Ch.3 — Phát hiện đặc trưng | Haar Cascade, Canny edge (Sobel, NMS, hysteresis), dlib 68 landmarks | Phát hiện khuôn mặt, phát hiện cạnh mắt, định vị đặc trưng khuôn mặt |
...
Kết quả quan sát khẳng định bộ tham số **EAR = 0.22, MAR = 0.30, HEAD_TILT = 15°** cho hiệu quả phát hiện tốt nhất. Ba phần tư giả thuyết được kiểm chứng là đúng, một phần tư được xác nhận đúng một phần. Hệ thống đáp ứng yêu cầu thời gian thực với độ trễ thấp và độ chính xác cao.

Pipeline được triển khai bằng Python với hơn 1.200 dòng mã nguồn, kiến trúc MVC rõ ràng. Notebook Jupyter kèm theo cung cấp ảnh minh họa trung gian cho từng bước xử lý và khảo sát tham số chi tiết.
...
| ALERT_COOLDOWN | 3 | Thời gian chờ giữa các cảnh báo (giây) |
...
| [SV3] | Phát hiện khuôn mặt & Landmark (Ch.3) | Haar Cascade, dlib HOG, DNN SSD face detection; dlib shape predictor 68 điểm landmark | Ch.3 — Phát hiện đối tượng, phát hiện đặc trưng |
